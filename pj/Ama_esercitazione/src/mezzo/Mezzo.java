...
public class Mezzo {
	...
	public void passo() {
		...
		case USCITA:
			...
			if (this.posizione.equals(this.zonaDiRaccolta)) {
	...
	private void eseguiRaccolta() {
		...
		final Posizione corrente = this.posizione;
	...
	public Posizione tentaSpostamentoVersoPosizione(Posizione obiettivo) {
		if (obiettivo==null)
			return this.posizione; // don't move
		
		final Posizione nuovaPos = this.posizione.traslazioneUnitariaVerso(obiettivo);
	...
	private Rifiuto raccogliRifiuto() {
		final Rifiuto rifiuto = this.citta.getRifiuto(this.posizione);
		if (rifiuto!=null) {
	...
	private boolean inCentroDiRaccolta() {
		return this.posizione.equals(this.citta.getCentroDiRaccolta().getPosizione());
	}
	@Override
	public String toString() {
		String s = this.getClass() + " " + this.getPolitica().getId();
		return s;
	}
}
